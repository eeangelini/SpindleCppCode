...
void setToBasePos() {
  ...
  for (unsigned i = 0; i < MT_numb_M; ++i) {
    ...
    float_T slope=tan(t_M);
    float_T yint=basePosM[1]-(slope*basePosM[0]);
    float_T elly=(yint*pow(R2_max,2)+R2_max*sqrt(pow(R1_max,4)*pow(slope,4)+pow(R1_max,2)*pow(R2_max,2)*pow(slope,2)-pow(R1_max,2)*pow(slope,2)*pow(yint,2)))/(pow(R1_max,2)*pow(slope,2)+pow(R2_max,2));
    float_T ellx=(elly-yint)/slope;
    float_T elldist=sqrt((ellx-basePosM[0])*(ellx-basePosM[0])+(elly-basePosM[1])*(elly-basePosM[1]));
    float_T r_M=elldist*randMR;
//    float_T r_M=elldist;
...
//    float_T r_D    = sqrt(randDR); //Random Radius of MT;
    ...
    float_T slope=tan(t_D);
    float_T yint=basePosD[1]-(slope*basePosD[0]);
    float_T elly=(yint*pow(R2_max,2)-R2_max*sqrt(pow(R1_max,4)*pow(slope,4)+pow(R1_max,2)*pow(R2_max,2)*pow(slope,2)-pow(R1_max,2)*pow(slope,2)*pow(yint,2)))/(pow(R1_max,2)*pow(slope,2)+pow(R2_max,2));
    float_T ellx=(elly-yint)/slope;
    float_T elldist=sqrt((ellx-basePosD[0])*(ellx-basePosD[0])+(elly-basePosD[1])*(elly-basePosD[1]));
    float_T r_D=elldist*randDR;
//    float_T r_D=elldist;
