...
//EA: write final time & pos
void writeFinalTime(const float_T t) {
    /* writeFinalTime: A function that writes the end time & position data.
     * Inputs:
     *   const float_T t: The simulation time at which the points are being
     *     written.
     * Output: (none)
     */
    file << proNucPos << "," << psi << "," << t << endl;
}
...
void mtForceCalcM()
{
    ...
    for (size_t i=0; i < MT_numb_M; ++i) {
        ...
        for (size_t j = 1; j <= numRegions; ++j){
            if (angle < regionAngles[j] && angle >= regionAngles[j-1]) { //i.e. which region?
                if (regionForceMultipliers[j-1]<0)
                    //multiplier = regionForceMultipliers[j-1]*F_MT_push;
                    //EA: for length dependent push BAND forces
                    xdist = (MT_Pos_M[i][0] - basePosM[0])*(MT_Pos_M[i][0] - basePosM[0]);
                    ydist = (MT_Pos_M[i][1] - basePosM[0])*(MT_Pos_M[i][1] - basePosM[1]);
                    L = sqrt(xdist + ydist); //MT length
                    multiplier = regionForceMultipliers[j-1]*F_MT_push*L; //Push force multiplier*L
                if (regionForceMultipliers[j-1]>0){
                    ...
                    //EA: for length dependent pull forces
                    if (MT_Contact_M[i]>0) {
                        xdist = (MT_Pos_M[i][0] - basePosM[0])*(MT_Pos_M[i][0] - basePosM[0]);
                        ydist = (MT_Pos_M[i][1] - basePosM[0])*(MT_Pos_M[i][1] - basePosM[1]);
                        L = sqrt(xdist + ydist); //MT length
                        multiplier = regionForceMultipliers[j-1]*F_MT_pull*L; //Pull force multiplier*L
                    }
                    //EA: for length dependent push forces
                    if (MT_Contact_M_push[i]>0) {
                        xdist = (MT_Pos_M[i][0] - basePosM[0])*(MT_Pos_M[i][0] - basePosM[0]);
                        ydist = (MT_Pos_M[i][1] - basePosM[0])*(MT_Pos_M[i][1] - basePosM[1]);
                        L = sqrt(xdist + ydist); //MT length
                        multiplier = -1*regionForceMultipliers[j-1]*F_MT_push*L; //Push force multiplier*L
                    }
                }
            }
        }
        ...
        for (size_t j = 1; j <= numRegions; ++j) {
            if (angle < regionAngles[j] && angle >= regionAngles[j-1]) { //i.e. which region?
                if (regionForceMultipliers[j-1]<0)
                    //multiplier = regionForceMultipliers[j-1]*F_MT_push;
                    //EA: for length dependent push BAND forces
                    xdist = (MT_Pos_D[i][0] - basePosD[0])*(MT_Pos_D[i][0] - basePosD[0]);
                    ydist = (MT_Pos_D[i][1] - basePosD[0])*(MT_Pos_D[i][1] - basePosD[1]);
                    L = sqrt(xdist + ydist); //MT length
                    multiplier = regionForceMultipliers[j-1]*F_MT_push*L; //Push force multiplier*L
                if (regionForceMultipliers[j-1]>0){
                    //multiplier = F_MT_pull;
...
void updatePNPos() {
    ...
    if (translation) {
        ...
        proNucPos += (1.0/Eta)*Tau*force + sqrt(2*Dp*Tau)*randomVec;
        basePosM = proNucPos + proNucRad;
        basePosD = proNucPos - proNucRad;
    }
    
    float_T randomtorque = sqrt(2*Dpr*Tau)*stdNormalDist(generator);
    
    torque = torque_M + torque_D;
    
    psi += (1/Mu)*torque*Tau + randomtorque;
}
...
float_T probContact(const float_T ang) {
    ...
    for (size_t i = 1; i <= numRegions; i++) {
        if (ang < regionAngles[i]) {
            if (regionForceMultipliers[i-1]<0) return 0;
            else return regionProbabilities[i-1];
        }
    }
...
void runModel(bool writeAllData, bool writeTempData) {
    ...
    for (float_T t=0; t <= Duration; t += Tau) {
        //EA: this is where we insert check for absorbing boundaries
        if (psi >= pi/2.0 + 0.9*(pi/2.0) || psi <= pi/2.0 - 0.9*(pi/2.0)) {
            writeFinalTime(t);
            break;
        }
        ...
        for (size_t i = 0; i < MT_numb_M; ++i) {
            ...
            if (mag_M < 0.2) respawnMT(M_CENTROSOME, vecM, i, envelopeM); //ATD
            
            if ((MT_Contact_M[i]==0 || MT_Contact_M_push[i]==0)&& ((thetaM < envelopeM[0]-0.05*pi) || (thetaM > envelopeM[1]+0.05*pi))) //ATD
        ...
        for (size_t i = 0; i < MT_numb_D; ++i) {
            ...
            if (mag_D < 0.2) respawnMT(D_CENTROSOME, vecD, i, envelopeD);
            
            if ((MT_Contact_D[i]==0 || MT_Contact_D_push[i]==0) && ((thetaD < envelopeD[0]-0.05*pi) || (thetaD > envelopeD[1]+0.05*pi))) //ATD
